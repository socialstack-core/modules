...
namespace Api.Permissions
{
	...
	/// <summary>
	/// Checks if a field in an arg contains a given value.
	/// </summary>
	public partial class FilterFieldContains: FilterNode
	{
		/// <summary>
		...
		public FilterFieldContains(Type type, string field)
		{
			Type = type;
			Field = field;
		...
		public override bool IsGranted(Capability capability, Context token, object[] extraObjectsToCheck)
		{
			...
			if (firstArgAsStr != null && firstArgAsStr.Contains(Value))
			{
				return true;
			}
	...
	public partial class Filter
	{
		...
		/// <summary>
		/// Adds a filter node which checks if the value at the given argIndex contains the given value.
		...
		/// <returns></returns>
		public Filter ContainsArg(System.Type type, string fieldName, int argIndex = 0)
		{
			return Add(new FilterFieldContains(type, fieldName)
			{
				AlwaysArgMatch = true,
				ArgIndex = argIndex
			});
		...
		/// <param name="argIndex"></param>
		/// <returns></returns>
		public Filter Contains(System.Type type, string fieldName, string value, int argIndex = 0)
		{
			return Add(new FilterFieldContains(type, fieldName)
			{
				Value = value,
