...
If this is your first time using a database on your own computer, it is recommended to also install MySQL Workbench so you can edit the database contents. It is not the only tool for this though so otherwise feel free to use your favourite UI.
...
This project uses the "main branch with feature flags and feature branches" commit strategy. This is similar to what Google does although each project is in its own repository rather than their monorepo strategy. This means you typically commit to the main branch. If you are adding a new feature then it is added behind some sort of configuration option and is off by default.
...
The aim of this strategy is to minimise merge overhead whilst also keeping the main branch in a deployable state at all times.
...
During development, most content is typically actually installed or generated by code such that you don't need to do anything to a database during deployment.
