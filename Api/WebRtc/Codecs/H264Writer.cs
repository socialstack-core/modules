...
/// <summary>
/// Based on https://github.com/BogdanovKirill/RtspClientSharp/blob/master/RtspClientSharp/MediaParsers/H264VideoPayloadParser.cs 
...
public class H264Writer
{
	...
	/// <returns></returns>
	public static FileStream Start(string filename)
	{
		var sps_pps = new byte[] {
			// Sps
	...
	/// <param name="fs"></param>
	public static void Write(byte[] buffer, int paddingSize, int index, int payloadSize, ushort seq, uint timestamp, bool markerBit, FileStream fs)
	{
		// timestamp -= firstTimestamp;
		var rtpPayload = new byte[payloadSize - paddingSize];
		Array.Copy(buffer, index, rtpPayload, 0, rtpPayload.Length);
		
		var writer = new H264Writer();
		writer.ProcessRTPPayload(rtpPayload, seq, timestamp, markerBit, out _, fs);
		fs.Flush();
	}
	...
	/// <returns></returns>
	public virtual bool ProcessRTPPayload(byte[] rtpPayload, ushort seqNum, uint timestamp, bool markbit, out bool isKeyFrame, Stream intoStream)
	{
		...
		if (nal_units != null)
		{
			...
			{
				intoStream.WriteByte(0);
				intoStream.WriteByte(0);
				intoStream.WriteByte(0);
				intoStream.WriteByte(1);
				intoStream.Write(nal, 0, nal.Length);
			}
			return true;
		}
	...
	/// <param name="isKeyFrame"></param>
	/// <returns></returns>
	public virtual List<byte[]> ProcessRTPPayloadAsNals(byte[] rtpPayload, ushort seqNum, uint timestamp, bool markbit, out bool isKeyFrame)
	{
	...
	/// 
	/// </summary>
	/// <param name="rtp_payload"></param>
	/// <param name="seqNum"></param>
	/// <param name="rtp_timestamp"></param>
	/// <param name="rtp_marker"></param>
	/// <param name="isKeyFrame"></param>
	/// <returns></returns>
	protected virtual List<byte[]> ProcessH264Payload(byte[] rtp_payload, ushort seqNum, uint rtp_timestamp, bool rtp_marker, out bool isKeyFrame)
	{
