...
namespace Api.CanvasRenderer
{
	...
	public class UIBundle
	{
		...
		/// </summary>
		public UIBundle(
			string rootPath, string packDir, TranslationService translations, LocaleService locales, FrontendCodeService frontend, GlobalSourceFileMap globalFileMap, bool minify)
		{
			RootName = rootPath;
		...
		/// <summary>
		/// This bundles meta.json file in prebuild mode.
		/// </summary>
		public PrebuiltMeta PrebuiltMeta;
		...
		{
			// Get file metadata first - if it's not an in-mem file and the cache is active.
			var cache = GlobalFileMap.Cache;

			var fileMeta = cache == null ? null : GetFileMeta(file.Path, file.RawSource);

			if (cache != null && cache.ScssHeaderChanged(GlobalFileMap))
			{
				// Can't use the cache for this call as the header changed.
				// Need the fileMeta to be not null though as that will drive the change indicator.
				cache.Miss++;
				cache = null;
			}

			var cachedFile = cache == null ? null : cache.GetFile(this, file.Path);

			if (cachedFile != null && !cache.FileChanged(cachedFile, fileMeta))
			{
				cache.Hit++;
				file.Failure = null;
				file.TranspiledContent = cachedFile.Content;
				file.ModifiedTicksTs = fileMeta.Value.LastWriteUtc;
				file.FileSize = fileMeta.Value.Length;
			...
			var scssHeader = GlobalFileMap.GetScssGlobals();

			try
			{
				var rawContent = TryReadFile(file.Path, file.RawSource);
				var engine = GlobalFileMap.GetBuildEngine();

				// Transform the SCSS now:
				var transpiledCss = engine.Invoke(
					"transformScss",
					rawContent,
					scssHeader,
					file.Path,
				...
				if (fileMeta != null)
				{
					file.FileSize = fileMeta.Value.Length;
					file.ModifiedTicksTs = fileMeta.Value.LastWriteUtc;
					GlobalFileMap.Cache.Changed = true;
				}
			}
			catch (Microsoft.ClearScript.ScriptEngineException e)
			{
				...
				else
				{
					...
					if (titleEnd != -1)
					{
						...
						{
							description += "\n\n⚠ Line numbers off by ~" + GlobalFileMap.GetScssHeaderLineCount() + " (the length of your globals)";
						}
		...
		internal static uint CalcCrc32(string str)
		{
			...
			{
				// Apply current char (naively treats it as a single byte here -
				// however the in-memory generated TS files this operates on are always ascii):
				var chr = str[i++];
		...
		private FileMeta? GetFileMeta(string path, string rawSource) {
			if (rawSource != null)
			{
				// Ram files use CRC32 as their mod time.
				return new FileMeta()
				{
					Length = rawSource.Length,
					LastWriteUtc = CalcCrc32(rawSource)
				};
			}
			...
			for (var i = 0; i < 20; i++)
			{
				try
				{
					...
					return new FileMeta() {
						...
						LastWriteUtc = fileInfo.LastWriteTimeUtc.Ticks
					};
				}
				catch (Exception)
				{
		...
		private string TryReadFile(string path, string ramContent) {
			if (ramContent != null)
			{
				// It's a virtual file - use that instead.
				return ramContent;
			}
	...
	{
		/// <summary>
		/// The source cache (if there is one active).
		/// </summary>
		public UIBuildCache Cache;
		...
		/// </summary>
		/// <param name="frontendCodeService"></param>
		public GlobalSourceFileMap(FrontendCodeService frontendCodeService)
		{
			_frontendService = frontendCodeService;
		}
		...
		/// </summary>
		/// <returns></returns>
		public V8ScriptEngine GetBuildEngine()
		{
			var engine = _engine;
			...
			// Spawn one:
			engine = _frontendService.GetBuildEngine();
			_engine = engine;
		...
		private uint ScssHeaderCrc32;

		/// <summary>
		/// Gets the SCSS CRC32.
		/// </summary>
		/// <returns></returns>
		public uint GetScssCrc()
		{
			return ScssHeaderCrc32;
		}
		...
		/// <summary>
		/// Gets the SCSS globals line count.
		/// </summary>
		/// <returns></returns>
		public int GetScssHeaderLineCount()
		{
			return ScssHeaderLineCount;
		}

		/// <summary>
		/// Builds (or rebuilds) the global SCSS header.
		/// </summary>
		public void ConstructScssHeader()
		{
			Sort();

			var sb = new StringBuilder();

			foreach (var kvp in SortedGlobalFiles)
			{
			...
			ScssHeaderCrc32 = UIBundle.CalcCrc32(ScssHeader);
		}
	...
	public class SourceFile
	{
		...
		/// <summary>
		/// The unix timestamp of the last modified time.
		/// </summary>
		public long ModifiedTicksTs;
		...
		public static SourceFileType GetTypeMeta(string sourcePath, string filePath, out string fileName, out string fileNameNoType, out string fileType, out string relativePath)
		{
			...
			var isStatic = filePath.IndexOf(System.IO.Path.DirectorySeparatorChar + "static" + System.IO.Path.DirectorySeparatorChar) != -1;

			var tidyFileType = isStatic ? SourceFileType.None :
	...
	/// <summary>
	/// Info from the filesystem about the file.
	/// </summary>
	public struct FileMeta
	{
		/// <summary>
		/// The file last write time (ticks)
		/// </summary>
