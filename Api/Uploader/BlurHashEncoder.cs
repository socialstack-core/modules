...
public static class BlurHashEncoder
{
	...
	public static string Encode(ImageMagick.MagickImage image, bool isWide)
	{
		...
		{
			// bad size. Must have a resizer size smaller if you want to use blurhash.
			return null;
		}

		var fmt = image.Format;
		image.Format = ImageMagick.MagickFormat.Rgb;
		var pixels = image.ToByteArray();
		image.Format = fmt;

		var xComponents = isWide ? 4 : 3;
		var yComponents = isWide ? 3 : 4;
