    ...
    public static partial class AppSettings
    {
        ...
        private static AppSettingsFile LoadFromJsonFile(string path)
        {
	        ...
	        if (Services.BuildHost == "xunit")
	        {
		        ...
		        return new AppSettingsFile(Path.Combine(fullPath, path));
	        }
	        else
	        {
        ...
        private static void LoadPublicUrls()
        {
            ...
            if (urlToken.Type == JTokenType.String)
            {
                ...
                _publicUrlByLocaleId = new string[] {
						NormalisePublicUrl(urlToken.ToString())
                    };
            }
            else if (urlToken.Type == JTokenType.Array)
            {
                ...
                foreach (var entry in urlArray)
                {
                    ...
                    set[entryLocaleId - 1] = NormalisePublicUrl(url);
                }
        ...
        /// <summary>
        /// Ensure publicUrl never ends with a trailing slash.
        /// </summary>
        /// <param name="url"></param>
        /// <returns></returns>
        public static string NormalisePublicUrl(string url)
        {
            ...
            if(string.IsNullOrEmpty(url))
            {
                // It can have been null the first time, or now empty after a trim.
                return url;
            }
        ...
        {
            var envValue = System.Environment.GetEnvironmentVariable("SOCIALSTACK_" + name);
        ...
        public static int GetInt32(string name, int defaultValue)
        {
            ...
            var textValue = GetString(name, null);
