...
public partial class NGINXConfigFile : NGINXContext
{
    ...
    /// </summary>
    public void WriteToFile(string nginxConfigPath)
    {
        // Write nginx.conf to the working directory (alongside the socket files on a deployed Linux server).
        File.WriteAllText($"{nginxConfigPath}/nginx.conf", ToString());

        if (_configurableContext != null)
        {
            File.WriteAllText($"{nginxConfigPath}/nginx-url-config.conf", _configurableContext.ToString());
        }

        try
        {
            if (File.Exists("/etc/nginx/sites-enabled/default"))
            {
                File.Delete("/etc/nginx/sites-enabled/default");
            }

            if (!File.Exists($"/etc/nginx/sites-enabled/nginx-{Services.Environment}.conf"))
            {
                File.CreateSymbolicLink($"/etc/nginx/sites-enabled/nginx-{Services.Environment}.conf", $"{nginxConfigPath}/nginx.conf");
            }
        }
        catch (Exception e)
        {
            Log.Error("Nginx", e, $"Failed to update nginx symbolic link");
        }
    }
    ...
    public void SetupDefaults(string nginxConfigPath, Dictionary<string, DomainCertificateLocales> certInfo = null)
    {
        ...
        httpToHttpsMain
            .AddLocationContext("~ ^/.well-known/acme-challenge/")
        ...
        cfgContext
            .AddLocationContext("~ ^/content-private/")
                ...
                .AddDirective("proxy_http_version", "1.1") // NGINX proxy pass config - use HTTP/1.1
                .AddDirective("proxy_set_header", "Host $host") // Set the Host response header
				.AddDirective("proxy_set_header", "X-Real-IP $remote_addr") // Passing the IP
                .AddDirective("proxy_set_header", "X-Forwarded-For $proxy_add_x_forwarded_for") // All hop IPs
				.AddDirective("proxy_cache_bypass", "$http_upgrade")
                .AddDirective("proxy_set_header", "Connection keep-alive")
