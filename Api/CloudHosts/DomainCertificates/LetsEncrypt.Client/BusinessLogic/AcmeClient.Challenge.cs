...
namespace LetsEncrypt.Client
{
    public partial class AcmeClient
    {
        ...
        /// <summary>
        /// </summary>
        public async Task<Challenge> GetChallengeAsync(Account account, string challengeUrl)
        {
            var url = new Uri(challengeUrl);
            var nonce = await GetNonceAsync();

            var signedData = account.Signer.Sign(null, account.Location, url, nonce);
            return await PostAsync<Challenge>(url, signedData);
        }

        /// <summary>
        /// </summary>
        public async Task ValidateChallengeAsync(Account account, string challengeUrl)
        {
            var url = new Uri(challengeUrl);
            var nonce = await GetNonceAsync();

            var signedData = account.Signer.Sign(new { }, account.Location, url, nonce);
            await PostAsync<Challenge>(url, signedData);
        }
        ...
        /// <returns></returns>
        public async Task<Authorization> GetAuthorizationAsync(Account account, Uri authorizationId)
        {
            var nonce = await GetNonceAsync();
        ...
        /// <returns></returns>
        public string GetChalangeKey(Account account, string token)
        {
