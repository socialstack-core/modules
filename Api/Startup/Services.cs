...
namespace Api.Startup
{
    ...
    public static class Services
    {
        ...
        /// <returns></returns>
        public static List<HostMapping> GetHostMappings(string hostName)
        {
            List<HostMapping> hostMappings = null;

            if (HostNameMappings != null)
            {
                foreach (var hostMapping in HostNameMappings)
                {
                    var reg = new Regex(hostMapping.Hostname, RegexOptions.IgnoreCase);

                    if (reg.Match(hostName).Success)
                    {
                        if (hostMappings == null)
                        {
                            hostMappings = new List<HostMapping>();
                        }
        ...
        public static bool HasHostType(string typeName)
        {
            if (HostTypes == null)
            {
                // has none so assume all 
                return true;
            }

            return HostTypes.Contains(typeName);
        }
        ...
        /// <returns></returns>
        public static bool HasHostType(List<string> typeNames)
        {
            if (HostTypes == null)
            {
                // has none so assume all 
            ...
            return HostTypes.Any(typeNames.Contains);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static List<string> HostTypes
        {
            get
            {
                return HostMappings != null ? HostMappings.Select(s => s.HostType).ToList() : null;
            }
        }
		...
		/// Useful for when in entity classes etc.
		/// </summary>
		/// <param name="t">The services interface.</param>
		/// <returns></returns>
