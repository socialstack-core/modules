			...
			Events.DatabaseDiffBeforeAdd.AddEventListener(async (Context ctx, FieldMap fieldMap, Type typeInfo, Schema newSchema) => {
				...
				for (var fm=fieldMap.Fields.Count-1; fm >= 0; fm--)
				{
					...
					for (var i = 0; i < locales.Count; i++)
					{
						...
						{
							Log.Warn("locale", "[WARN] Ignoring locale #" + locales[i].Id + " because it has a blank code. This message will spam so you'll want to delete locale or give it a code.");
							continue;
						}
