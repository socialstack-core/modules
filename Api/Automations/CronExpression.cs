...
/*
 * This is a modified (to remove allocations) variant of CronExpression from Quartz.NET
 * Its original license is below.
 *
...
public class CronExpression
{
	...
	/// Seconds.
	/// </summary>
	protected List<int> seconds = null!;

	/// <summary>
	/// minutes.
	/// </summary>
	protected List<int> minutes = null!;

	/// <summary>
	/// Hours.
	/// </summary>
	[NonSerialized] protected List<int> hours = null!;

	/// <summary>
	/// Days of month.
	/// </summary>
	[NonSerialized] protected List<int> daysOfMonth = null!;

	/// <summary>
	/// Months.
	/// </summary>
	[NonSerialized] protected List<int> months = null!;

	/// <summary>
	/// Days of week.
	/// </summary>
	[NonSerialized] protected List<int> daysOfWeek = null!;

	/// <summary>
	/// Years.
	/// </summary>
	[NonSerialized] protected List<int> years = null!;
	...
	protected virtual void AddToSet(int val, int end, int incr, int type)
	{
		...
		{
			incr = 1;
			SortedAdd(data, AllSpec); // put in a marker, but also fill values
		}
	...
	/// <returns></returns>
	protected virtual List<int> GetSet(int type)
	{
		switch (type)
		{
			case Second:
	...
	private static void SortedAdd(List<int> set, int val)
	{
		...
		// Probably going on the end most of the time, so we'll go backwards from the end.
		...
		{
			// Does it want to go here?
			if (set[i] > val)
			{
				// "flip" them over by moving set[i]
				set[i + 1] = set[i];
				set[i] = val;
			}
		}
	...
	private static bool GetNextInSet(List<int> set, ref int value)
	{
		...
		// Either the given value is the last in the set or it wasn't in there.
		if (set.Count > 0)
		{
			// Set as first:
			value = set[0];
	...
	public virtual DateTimeOffset? GetTimeAfter(DateTimeOffset afterTimeUtc)
	{
		...
		while (!gotOne)
		{
			...
			// get second.................................................
			if (!GetNextInSet(seconds, ref sec))
			{
				// Sec has been set to the first entry.
				d = d.AddMinutes(1);
			}
			...
			// get minute.................................................
			var origMin = min;
			if (GetNextInSet(minutes, ref min))
			{
				t = origMin;
			}
			else
			{
				// min has been set to first one.
				hr++;
			}
			...
			if (dayOfMSpec && !dayOfWSpec)
			{
				...
				int nextInDomSet = day;
				bool found = GetNextInSet(daysOfMonth, ref nextInDomSet);
			...
			else if (dayOfWSpec && !dayOfMSpec)
			{
				...
				else if (everyNthWeek != 0)
				{
					...
					// d-o-w
					var nextDow = cDow;
					if (GetNextInSet(daysOfWeek, ref nextDow))
					{
						dow = nextDow;
					}
			...
			// get month...................................................
			var origMon = mon;
			if (GetNextInSet(months, ref mon))
			{
				t = origMon;
			}
			else
			{
				// Mon set to first month
				year++;
			}
			...
			if (GetNextInSet(years, ref year))
			{
				t = origYear;
			}
			else
	...
	public bool Equals(CronExpression other)
	{
		...
		return Equals(other.CronExpressionString, CronExpressionString);
	}
	...
	public override int GetHashCode()
	{
		unchecked
		{
			return (CronExpressionString != null ? CronExpressionString.GetHashCode() : 0);
		}
	}
}
