  ...
  public class ExpressionDescriptor
  {
    ...
    protected string GetSecondsDescription()
    {
      string description = GetSegmentDescription(
           ...
           if (int.TryParse(s, out i))
           {
             return s == "0"
                    ...
                    : (i < 20)
						...
						: "at {0} seconds past the minute" ?? "at {0} seconds past the minute";
         ...
         (s => ", minutes {0} through {1}" ?? ", {0} through {1}")
         );
    ...
    protected string GetMinutesDescription()
    {
      ...
      string description = GetSegmentDescription(
            ...
            if (int.TryParse(s, out i))
            {
              return s == "0" && secondsExpression == ""
                      ...
                      : (int.Parse(s) < 20)
						  ...
						  : "at {0} minutes past the hour" ?? "at {0} minutes past the hour";
            }
            else
            {
          ...
          getRangeFormat: (s => ", minutes {0} through {1}" ?? ", {0} through {1}")
      );
    ...
    protected string GetMonthDescription()
    {
      string description = GetSegmentDescription(
         ...
         (s => ", month {0} through month {1}" ?? ", {0} through {1}"),
         (s => ", only in {0}"),
         (s => ", month {0} through month {1}" ?? ", {0} through {1}")
      );
      ...
      switch (expression)
      {
        ...
        default:
          ...
          if (weekDayNumberMatches.IsMatch(expression))
          {
            ...
            string dayString = dayNumber == 1 ? "first weekday" :
    ...
    private string GetYearDescription()
    {
      string description = GetSegmentDescription(m_expressionParts[6],
         ...
         (s => ", year {0} through year {1}" ?? ", {0} through {1}"),
         (s => ", only in {0}"),
         (s => ", year {0} through year {1}" ?? ", {0} through {1}")
      );
