...
#pragma warning restore CS8981 // Lowercase names
{
	...
	private ustring(params char[] chars)
	{
		if (chars == null)
			throw new ArgumentNullException(nameof(chars));
		buffer = System.Text.Encoding.UTF8.GetBytes(chars);
	}
	...
	class ByteBufferUString : ustring {
		...
		public ByteBufferUString (byte [] buffer) : base(buffer)
		{
		}

		public ByteBufferUString (uint rune) : base(rune)
		{
		}

		public ByteBufferUString (string str) : base(str)
		{
		}

		public ByteBufferUString (params char [] chars) : base(chars)
		{
		}

		public override int Length => buffer.Length;
		public override byte this [int index] => buffer [index];
	...
	class ByteRangeUString : ustring {
		...
		public ByteRangeUString (byte [] buffer, int start, int count) : base(buffer)
		{
			if (start < 0)
		...
		public override byte this [int index] => buffer [start + index];
		...
		public override string ToString ()
		{
			return System.Text.Encoding.UTF8.GetString (buffer, start, count);
		}

		internal override int RealIndexByte(byte b, int offset)
		{
			var t = count + start;
			for (var i = offset + start; i < t; i++)
			{
				if (buffer[i] == b)
					return i - start;
			}
	...
	/// <summary>
	/// Determines whether a specified instance of <see cref="ustring"/> is equal to another specified <see cref="ustring"/>, this means that the contents of the string are identical
	/// </summary>
	/// <param name="a">The first <see cref="ustring"/> to compare.</param>
	/// <param name="b">The second <see cref="ustring"/> to compare.</param>
	/// <returns><c>true</c> if <c>a</c> and <c>b</c> are equal; otherwise, <c>false</c>.</returns>
	public static bool operator == (ustring a, ustring b)
	{
		// Virtually the same as the built in dotnet string.
	...
	/// <summary>
	/// Determines whether a specified instance of <see cref="ustring"/> is not equal to another specified <see cref="ustring"/>.
	/// </summary>
	/// <param name="a">The first <see cref="ustring"/> to compare.</param>
	/// <param name="b">The second <see cref="ustring"/> to compare.</param>
	...
	/// <summary>
	/// Determines whether the specified <see cref="object"/> is equal to the current <see cref="T:ustring"/>.
	/// </summary>
	/// <param name="obj">The <see cref="object"/> to compare with the current <see cref="T:ustring"/>.</param>
	/// <returns><c>true</c> if the specified <see cref="object"/> is equal to the current <see cref="T:ustring"/>;
	...
	public int IndexOfAny (ustring chars)
	{
		...
		var blen = Length;
		if (blen > 8) {
			Span<byte> aset = stackalloc byte[32];
	...
	struct AsciiSet
	{
		public static bool MakeAsciiSet (ref Span<byte> aset, ustring chars)
		{
			var n = chars.Length;
			for (int i = 0; i < n; i++) {
				var c = chars [i];
				...
				// Bottom 3 bits are stored in the byte.
				aset[c >> 3] |= (byte)(1 << (c & 7));
			}
		...
		public static bool MakeAsciiSet (ref Span<byte> aset, uint [] runes)
		{
			var n = runes.Length;
			for (int i = 0; i < n; i++)
			{
		...
		public static bool Contains (ref Span<byte> aset, byte b)
		{
			return (aset[b >> 3] & (1 << (b & 7))) != 0;
		}
	}

	/// <summary>
	/// Concatenates the contents of two <see cref="ustring"/> instances.
	/// </summary>
	/// <param name="u1">The first <see cref="ustring"/> to add, can be null.</param>
	/// <param name="u2">The second <see cref="ustring"/> to add, can be null.</param>
	...
	public ustring TrimStart (ustring cutset)
	{
		...
		Span<byte> aset = stackalloc byte[32];
		var useAsciiSet = AsciiSet.MakeAsciiSet(ref aset, cutset);
		...
		{
			(var rune, var size) = Utf8.DecodeRune(this, i, i - blen);
			// if it is _not_ in cutset, stop at i.

			var matchResult = useAsciiSet ? rune < Utf8.RuneSelf && AsciiSet.Contains(ref aset, (byte)rune) : cutset.IndexOf(rune) >= 0;
			if (!matchResult)
			{
				substrStart = i;
				break;
		...
		return (substrStart == -1) ? this : this[substrStart, null];
	}
	...
	public ustring TrimEnd (ustring cutset)
	{
		...
		if (substrEnd >= 0 && this[substrEnd] >= Utf8.RuneSelf)
		{
			(var rune2, var wid2) = Utf8.DecodeRune(this[substrEnd, null]);
			substrEnd += wid2;
		}
		else
		{
