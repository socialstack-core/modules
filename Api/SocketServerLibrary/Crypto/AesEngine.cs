...
namespace Api.SocketServerLibrary.Crypto
{
	...
	public class AesEngine
	{
		...
		*/
		private uint[][] GenerateWorkingKey(byte[] key, bool forEncryption)
		{
			...
			switch (KC)
			{
				case 4:
					{
						uint t0 = LE_To_UInt32(key, 0); W[0][0] = t0;
						uint t1 = LE_To_UInt32(key, 4); W[0][1] = t1;
						uint t2 = LE_To_UInt32(key, 8); W[0][2] = t2;
						uint t3 = LE_To_UInt32(key, 12); W[0][3] = t3;
				...
				case 6:
					{
						...
						uint t2 = LE_To_UInt32(key, 8); W[0][2] = t2;
						uint t3 = LE_To_UInt32(key, 12); W[0][3] = t3;
						uint t4 = LE_To_UInt32(key, 16); W[1][0] = t4;
						uint t5 = LE_To_UInt32(key, 20); W[1][1] = t5;
				...
				case 8:
					{
						...
						uint t4 = LE_To_UInt32(key, 16); W[1][0] = t4;
						uint t5 = LE_To_UInt32(key, 20); W[1][1] = t5;
						uint t6 = LE_To_UInt32(key, 24); W[1][2] = t6;
						uint t7 = LE_To_UInt32(key, 28); W[1][3] = t7;
		...
		*/
		...
		/// <param name="outOff"></param>
		/// <returns></returns>
		public virtual int ProcessBlock(Span<byte> input, int inOff, byte[] output, int outOff)
		{
			if (forEncryption)
			{
				EncryptBlock(input, inOff, output, outOff);
		...
		/// <param name="outOff"></param>
		/// <returns></returns>
		public virtual int ProcessBlock(Span<byte> input, int inOff, Span<byte> output, int outOff)
		{
			if (forEncryption)
			{
				EncryptBlock(input, inOff, output, outOff);
		...
		/// <param name="output"></param>
		/// <param name="outOff"></param>
		public void EncryptBlock(Span<byte> input, int inOff, Span<byte> output, int outOff)
		{
			Span<uint> inout = stackalloc uint[4];
			inout[0] = LE_To_UInt32(ref input, inOff);
			inout[1] = LE_To_UInt32(ref input, inOff + 4);
			inout[2] = LE_To_UInt32(ref input, inOff + 8);
			inout[3] = LE_To_UInt32(ref input, inOff + 12);
			DoEncrypt(inout);
			UInt32_To_LE(inout[0], ref output, outOff);
			UInt32_To_LE(inout[1], ref output, outOff + 4);
			UInt32_To_LE(inout[2], ref output, outOff + 8);
			UInt32_To_LE(inout[3], ref output, outOff + 12);
		}
		...
		/// <param name="output"></param>
		/// <param name="outOff"></param>
		public void EncryptBlock(Span<byte> input, int inOff, byte[] output, int outOff)
		{
			Span<uint> inout = stackalloc uint[4];
			...
			UInt32_To_LE(inout[0], output, outOff);
			UInt32_To_LE(inout[1], output, outOff + 4);
			UInt32_To_LE(inout[2], output, outOff + 8);
			UInt32_To_LE(inout[3], output, outOff + 12);
		}
		...
		public void EncryptBlock(byte[] input, int inOff, byte[] output, int outOff)
		{
			...
			inout[0] = LE_To_UInt32(input, inOff);
			inout[1] = LE_To_UInt32(input, inOff + 4);
			inout[2] = LE_To_UInt32(input, inOff + 8);
			inout[3] = LE_To_UInt32(input, inOff + 12);
			DoEncrypt(inout);
		...
		private void DoEncrypt(Span<uint> inout)
		{
			uint[][] KW = WorkingKey;
			uint[] kw = KW[0];
			uint t0 = inout[0] ^ kw[0];
			uint t1 = inout[1] ^ kw[1];
			uint t2 = inout[2] ^ kw[2];

			uint r0, r1, r2, r3 = inout[3] ^ kw[3];
			int r = 1;
			while (r < ROUNDS - 1)
			{
				kw = KW[r++];
			...
			kw = KW[r];
			inout[0] = (uint)S[r0 & 255] ^ (((uint)S[(r1 >> 8) & 255]) << 8) ^ (((uint)s[(r2 >> 16) & 255]) << 16) ^ (((uint)s[(r3 >> 24) & 255]) << 24) ^ kw[0];
			inout[1] = (uint)s[r1 & 255] ^ (((uint)S[(r2 >> 8) & 255]) << 8) ^ (((uint)S[(r3 >> 16) & 255]) << 16) ^ (((uint)s[(r0 >> 24) & 255]) << 24) ^ kw[1];
			inout[2] = (uint)s[r2 & 255] ^ (((uint)S[(r3 >> 8) & 255]) << 8) ^ (((uint)S[(r0 >> 16) & 255]) << 16) ^ (((uint)S[(r1 >> 24) & 255]) << 24) ^ kw[2];
			inout[3] = (uint)s[r3 & 255] ^ (((uint)s[(r0 >> 8) & 255]) << 8) ^ (((uint)s[(r1 >> 16) & 255]) << 16) ^ (((uint)S[(r2 >> 24) & 255]) << 24) ^ kw[3];
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private uint LE_To_UInt32(ref Span<byte> bs, int off)
		{
			return (uint)bs[off]
				| (uint)bs[off + 1] << 8
				| (uint)bs[off + 2] << 16
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void UInt32_To_LE(uint n, ref Span<byte> bs, int off)
		{
			bs[off] = (byte)(n);
			bs[off+1] = (byte)(n >> 8);
			bs[off+2] = (byte)(n >> 16);
		...
		private void DoDecrypt(Span<uint> inout)
		{
			uint[][] KW = WorkingKey;
			uint[] kw = KW[ROUNDS];
			uint t0 = inout[0] ^ kw[0];
			...
			kw = KW[0];
			inout[0] = (uint)Si[r0 & 255] ^ (((uint)s[(r3 >> 8) & 255]) << 8) ^ (((uint)s[(r2 >> 16) & 255]) << 16) ^ (((uint)Si[(r1 >> 24) & 255]) << 24) ^ kw[0];
			inout[1] = (uint)s[r1 & 255] ^ (((uint)s[(r0 >> 8) & 255]) << 8) ^ (((uint)Si[(r3 >> 16) & 255]) << 16) ^ (((uint)s[(r2 >> 24) & 255]) << 24) ^ kw[1];
			inout[2] = (uint)s[r2 & 255] ^ (((uint)Si[(r1 >> 8) & 255]) << 8) ^ (((uint)Si[(r0 >> 16) & 255]) << 16) ^ (((uint)s[(r3 >> 24) & 255]) << 24) ^ kw[2];
			inout[3] = (uint)Si[r3 & 255] ^ (((uint)s[(r2 >> 8) & 255]) << 8) ^ (((uint)s[(r1 >> 16) & 255]) << 16) ^ (((uint)s[(r0 >> 24) & 255]) << 24) ^ kw[3];
		}
		...
		/// 
		/// </summary>
		public void DecryptBlock(Span<byte> input, int inOff, byte[] output, int outOff)
		{
			Span<uint> inout = stackalloc uint[4];
		...
		/// 
		/// </summary>
		public void DecryptBlock(Span<byte> input, int inOff, Span<byte> output, int outOff)
		{
			Span<uint> inout = stackalloc uint[4];
