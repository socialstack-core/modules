...
const Expander: React.FC<React.PropsWithChildren<ExpanderProps>> = ({
	...
	useEffect(() => {
		...
		*/
		...
		const expanderEl = expanderRef.current;

		if (expanderEl) {
			expanderEl.addEventListener("click", clickHandler);
			// expanderEl.addEventListener("toggle", toggleHandler);
		}

		return () => {

			if (expanderEl) {
				expanderEl.removeEventListener("click", clickHandler);
				// expanderEl.removeEventListener("toggle", toggleHandler);
			}
