 ...
 * @param locale given locale (e.g. "en-gb" or "gb")
...
function getContactLink(number: string, locale: string, link?: string) {
	...
	return hasPlus ? `tel:+${digits}` : `tel:${getDialingCode(locale)}${digits.replace(/^0/, '')}`;
}

/**
 * Return a string array of populated lines from the given address
 * @param addr
 * @returns
 */
const getAddressLines = (addr: Address) => {
	var current = [];
	addr.name && current.push(addr.name);
	...
	addr.county && current.push(addr.county);
	addr.postcode && current.push(addr.postcode);
	return current;
};

/**
 * Return populated address lines wrapped in <div>s for rendering 
 * @param addr
 */
const displayAddress = (addr: Address) => {
	return getAddressLines(addr).map(line => <div>{line}</div>);
};
