...
namespace Api.Comments
{
	...
	public class LiveEventListener
	{
		...
		{
			Events.Comment.AfterCreate.AddEventListener(async (Context context, Comment comment) => {
				...
				await _websocketService.Send(
					new WebSocketMessage<Comment>() {
						Type = "CommentCreate?ContentId=" + comment.ContentId,
						Entity = comment
					}
				);
