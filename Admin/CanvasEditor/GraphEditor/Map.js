...
export class MapInteractionControlled extends Component {
    ...
    */
    ...
    onWheel(e) {
        ...
        const scaleChange = -Math.sign(e.deltaY) * 0.05;

        const newScale = clamp(
            this.props.minScale,
            this.props.value.scale + scaleChange,
            this.props.maxScale
        );
    ...
    getScale() {
		// Round to nearest 1%
		var n = this.props.value.scale * 100;
		return Math.round(n);
    }
    ...
    renderControls() {
        ...
        if (this.props.onRenderControls) {
            return this.props.onRenderControls(this, step);
        }
    ...
    render() {
        const { showControls, nodes, heading, instructions, children } = this.props;
        ...
        var mapInteractionClass = hasNodes ? "map-interaction" : "map-interaction map-interaction--empty";
        ...
        */
        ...
        return (
            <div className={mapInteractionClass}
                ref={(node) => {
                    this.containerNode = node;
                }}
                onClickCapture={handleEventCapture}
                onTouchEndCapture={handleEventCapture}>
                {!hasNodes && <div className="map-interaction__internal">
                    <div className="map-interaction__hero">
                        <h2 className="map-interaction__heading">
                            {heading}
                        </h2>
                        <p className="map-interacton__instructions">
                            {instructions}
                        </p>
                    </div>
                </div>}
                {hasNodes && (children || undefined) && children({ translation, scale })}
...
class MapInteractionController extends Component {
    ...
    /*
      Handle the parent switching form controlled to uncontrolled or vice versa.
      This is at most a best-effort attempt. It is not guaranteed by our API
