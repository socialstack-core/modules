...
export default class MediaCenter extends React.Component {
	...
	renderEntry(entry) {
		...
		return <>
			<div className="media-center__list-item" title={entry.originalName}>
				...
				{/* not an image, link direct to preview */}
				{(!isImage || isVideo) && <>
					<a href={url} target="_blank" className="btn btn-link media-center__original-filename" data-clamp="2">
						{entry.originalName}
					</a>
				</>}
				...
				{isImage && !isVideo && <>
					<button type="button" className="btn btn-link media-center__original-filename" data-clamp="2"
						onClick={() => {
							this.setState({
	...
	renderUploadModal() {
		...
		var title = isNewMedia ? `Upload media` : `Edit media`;
