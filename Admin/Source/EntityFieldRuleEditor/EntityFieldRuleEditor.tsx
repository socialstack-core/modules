...
const EntityFieldRuleEditor: React.FC<EntityFieldRuleEditorProps> = ({ entity: initialEntity, role }) => {
	...
	const readRule = currentRule?.canRead ?? '';
	const writeRule = currentRule?.canWrite ?? '';
				...
				>
					...
					<Form
						...
						action={(values: ContentFieldAccessRule) => {
							const baseRule: Partial<ContentFieldAccessRule> = {
					...
					>
						<Input
							...
							label="Can read"
							defaultValue={["", "false", "true"].includes(readRule) ? readRule : "custom"}
						>
						...
						<Input
							...
							label="Can write"
							defaultValue={["", "false", "true"].includes(writeRule) ? writeRule : "custom"}
						>
							<option value="">{`Inherited`}</option>
							<option value="false">{`Always denied`}</option>
							<option value="true">{`Always granted`}</option>
