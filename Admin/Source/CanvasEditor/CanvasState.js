...
export default class CanvasState{
		...
		if(type){
			if(type.indexOf('/') != -1){
					...
					if(Array.isArray(node.r)){
						...
						for(var key in node.r){
							...
							if(Array.isArray(rt)){
								...
								var childContent = this.convertToNodesFromCanvas(rt);
								if (childContent.typeName) {
									resultRoot = { content: [childContent] };
								} else {
									resultRoot = childContent;
								}
							}
				...
				for(var k in roots){
					...
					// Must always be an array as the root content.
	...
	toCanvasJson(pretty, withoutIds){
		...
		// If it represents an empty canvas (the br placeholder), return empty.
		if (cfNode?.c?.t === 'p' &&
			cfNode?.c?.c?.t === 'br') {
			return '';
		}
