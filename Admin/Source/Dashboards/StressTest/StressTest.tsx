...
import ws, { Writer } from 'UI/Functions/WebSocket';
import Alert from 'UI/Alert';
import Button from 'UI/Button';

function requestFor(writer: Writer, cType: int, id: int) {
		writer.writeByte(5 as int);
		writer.writeUInt32(8 as int);
		writer.writeUInt32(cType);
		writer.writeUInt32(id);
}

function blockRequest(count: int, cType: int, id: int): Writer {
...
const WsStressTest: React.FC = (): React.ReactNode => {
	...
	const runWebsocket = (stages: number[], stageIndex: number) => {
		...
		const writer = blockRequest(blockSize as int, 1 as int, 1 as int);
		let c = 0;
		let total = 0;
		let target = 0;
		...
		<div>
			{running ? (
				...
				<>
					...
					<Button onClick={() => websocketAdvance()}>
